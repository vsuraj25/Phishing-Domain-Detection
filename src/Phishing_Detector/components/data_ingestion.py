    ...
    def download_file(self):
        try:
            if not os.path.exists(self.config.local_data_file_path):
                logger.info(f"Downloading source file from url{self.config.source_url} ...")
                request.urlretrieve(
                    url = self.config.source_url,
                    filename= self.config.local_data_file_path
                )
                logger.info(f"File downloaded and saved to {self.config.local_data_file_path} ...")
            else:
                logger.info(f"File already present at path {self.config.local_data_file_path}")
    ...
    def convert_to_csv(self):
        try:
            if not os.path.exists(self.config.processed_data_file_path):
                ...
                logger.info(f"File converted to csv and saved at {self.config.processed_data_file_path}")
            else:
                logger.info(f"File already present at path {self.config.processed_data_dir_path}")

            logger.info(f"{'>'*10} Data Ingestion Stage Completed Successfully! {'<'*10}")
        except Exception as e:
            raise e
