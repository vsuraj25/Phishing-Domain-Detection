...
class DataTransformation:
    ...
    def transform_data(self):
        try:
            if self.get_validation_status():
                ...
                data.drop(self.config.param_cols_to_drop, axis = 1, inplace=True)

                ## Simple Imputer
                logger.info(f"Filling null values using SimpleImputer(strategy = {self.config.param_imputer_strategy})...")
                ...
                logger.info(f"Transformed Data Saved at {self.config.transformed_data_file_path}...")
            else:
                logger.info(f'Data is not validated - Validation Status = {self.get_validation_status()}')
        except Exception as e:
            raise e
    ...
    def split_data(self):
        try:
            ...
            logger.info(f"{'>'*10} Data Transformation Stage Completed Successfully! {'<'*10}")
