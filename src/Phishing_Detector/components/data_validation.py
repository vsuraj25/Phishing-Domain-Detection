...
class DataValidation:
    ...
    def validate_data(self):
        ...
        try:
            ...
            validation_status = (
                val_cols and val_dtypes and val_total_cols and val_total_rows
            )

            logger.info(
                f"Saving Validation report at {self.config.validation_report_file_path}..."
            )
            report = {
                "is_cols_valid": val_cols,
            ...
            logger.info(
                f"Validation report saved at {self.config.validation_report_file_path}."
            )

            if validation_status:
                logger.info(
                    f"{'>'*10} Data Validation Stage Completed Successfully! {'<'*10}"
                )
            else:
                logger.error(e)
                raise DataNotValid
